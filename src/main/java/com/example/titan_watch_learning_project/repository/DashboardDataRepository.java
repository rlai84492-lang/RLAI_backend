...
////
////    /**
////     * MAX rows returned in a single list call.
////     * Prevents 504 timeouts when there are lakhs of records.
////     * Metrics are computed via COUNT(*) — not from these lists.
////     */
...
////    // ════════════════════════════════════════════════════════════
////
////    /** All flows, no date filter — legacy/fallback. Capped at MAX_SESSION_ROWS. */
...
////
////    /**
////     * ★ MAIN list query — flow + date range.
////     * Uses index: idx_bs_step_activity(current_step, last_activity).
////     * Key fix: uses >= and < instead of DATE() BETWEEN so the index is hit.
////     */
////    public List<DashboardResponse.SessionDto> findSessionsByFlowAndDate(
////            String flow, LocalDateTime from, LocalDateTime to) {
////
////        String sql =
////                "SELECT bs.id, bs.phone, bs.current_step, bs.is_active, " +
////                        "       bs.last_activity, bs.selected_collection, bs.selected_brand, " +
...
////                        "  AND bs.last_activity >= ? AND bs.last_activity < ? " +   // ← INDEX-FRIENDLY
////                        "ORDER BY bs.last_activity DESC " +
////                        "LIMIT " + MAX_SESSION_ROWS;
////
////        return jdbcTemplate.query(sql, new Object[]{flowToPattern(flow), from, to},
////                (rs, rn) -> { try { return mapSession(rs); } catch (Exception e) { throw new RuntimeException(e); }});
////    }
////
////    // ════════════════════════════════════════════════════════════
////    // ── SESSIONS — PAGINATED (ConversationsPage — no hardcoded cap)
////    // ════════════════════════════════════════════════════════════
////
////    /**
////     * ★ NEW — Returns ALL matching sessions, paginated. No 500-row cap.
////     *
////     * Server-side filters (all indexed):
////     *   flow        → current_step LIKE pattern   (idx_bs_step_activity)
////     *   from / to   → last_activity >= ? < ?       (idx_bs_step_activity)
////     *   collection  → selected_collection = ?
////     *   brand       → selected_brand = ?
////     *   step        → current_step = ?  (exact match — overrides LIKE pattern)
////     *   search      → c.name LIKE ? OR bs.phone LIKE ?
////     *
////     * sortField is whitelisted to prevent SQL injection.
////     * Max page size capped at 500 for safety.
////     *
////     * @return Map: { sessions, totalSessions, totalPages, currentPage, pageSize }
////     */
////    public Map<String, Object> findSessionsPaginated(
////            String flow,
...
////        }
////        // exact step overrides the LIKE pattern
...
////        } catch (Exception e) {
////            log.warn("findSessionsPaginated count failed: {}", e.getMessage());
////            total = 0L;
////        }
...
////        try {
////            sessions = jdbcTemplate.query(dataSql, dataParams.toArray(),
////                    (rs, rn) -> { try { return mapSession(rs); } catch (Exception ex) { throw new RuntimeException(ex); }});
////        } catch (Exception e) {
////            log.warn("findSessionsPaginated data failed: {}", e.getMessage());
...
////        result.put("currentPage",   Math.max(page, 0));
////        result.put("pageSize",      safeSize);
////        return result;
////    }
////
////    // ════════════════════════════════════════════════════════════
////    // ── SESSIONS — COUNT / METRICS (no data fetch) ─────────────
////    // ════════════════════════════════════════════════════════════
////
////    /**
////     * ★ KEY OPTIMIZATION: Computes ALL session metrics in ONE SQL query
////     * using SUM(CASE WHEN ...) — no Java-side counting, no full data fetch.
////     *
////     * Returns map with keys:
////     *   total, active, replied, catalogue, completed
////     *
////     * Before: loadDashboard fetched all 29k sessions just to call sessions.size()
////     * After:  One COUNT query returns all five numbers in <50ms
////     */
////    public Map<String, Long> getSessionMetrics(String flow, LocalDateTime from, LocalDateTime to) {
...
////
////            jdbcTemplate.query(sql, new Object[]{flowToPattern(flow), from, to}, rs -> {
...
////
////    /**
////     * ★ KEY OPTIMIZATION: All lead metrics in ONE SQL query.
////     *
////     * Returns map with keys:
////     *   total, callback, store_visit, new_count, converted
////     */
////    public Map<String, Long> getLeadMetrics(String flow, LocalDateTime from, LocalDateTime to) {
...
////
////            jdbcTemplate.query(sql, new Object[]{flow, flowToPattern(flow), from, to}, rs -> {
////                result.put("total",       rs.getLong("total"));
...
////
////    /**
////     * ★ MAIN leads list — flow + date range.
////     * Uses index: idx_leads_flow_created(flow, created_at).
////     * Key fix: >= and < instead of DATE() BETWEEN.
////     */
...
////                            "  AND l.created_at >= ? AND l.created_at < ? " +   // ← INDEX-FRIENDLY
////                            "ORDER BY l.created_at DESC " +
////                            "LIMIT " + MAX_LEAD_ROWS;
////
////            return jdbcTemplate.query(sql,
////                    new Object[]{flow, flowToPattern(flow), from, to},
////                    (rs, rn) -> { try { return mapLead(rs); } catch (Exception e) { throw new RuntimeException(e); }});
////        } catch (Exception e) {
...
////    }
////
////    /** Backward-compat overload (LocalDate → converts to LocalDateTime internally). */
////    public List<DashboardResponse.LeadDto> findLeadsByFlowAndDate(
////            String flow, LocalDate from, LocalDate to) {
////        return findLeadsByFlowAndDate(flow,
////                from.atStartOfDay(),
////                to.plusDays(1).atStartOfDay());
////    }
////
////    // ════════════════════════════════════════════════════════════
////    // ── COLLECTION SPLIT — aggregate, no list needed ───────────
////    // ════════════════════════════════════════════════════════════
////
////    /**
////     * Counts mens/womens/couples selections directly in SQL.
////     * Faster than loading sessions and iterating in Java.
////     */
...
////                    jdbcTemplate.queryForObject(sql, new Object[]{flowToPattern(flow), from, to},
...
////    // ════════════════════════════════════════════════════════════
////
////    /** Flow counts for left sidebar — uses index idx_bs_active. */
...
////
////    // ════════════════════════════════════════════════════════════
////    // ── DELIVERY METRICS — messages table ──────────────────────
////    // ════════════════════════════════════════════════════════════
////
////    public long countDeliveredMessages(String flow, LocalDateTime from, LocalDateTime to) {
////        try {
////            String sql =
////                    "SELECT COUNT(*) FROM messages m " +
////                            "JOIN bot_sessions bs ON bs.phone = m.phone " +
////                            "WHERE bs.current_step LIKE ? " +
////                            "  AND m.status = 'DELIVERED' AND m.direction = 'OUTBOUND' " +
////                            "  AND m.sent_at >= ? AND m.sent_at < ?";
////            Long c = jdbcTemplate.queryForObject(sql, Long.class, flowToPattern(flow), from, to);
////            return c != null ? c : 0L;
////        } catch (Exception e) {
...
////    }
////
////    public long countSentMessages(String flow, LocalDateTime from, LocalDateTime to) {
////        try {
////            String sql =
////                    "SELECT COUNT(*) FROM messages m " +
////                            "JOIN bot_sessions bs ON bs.phone = m.phone " +
...
////                    (rs, rn) -> { try { return mapActivityEvent(rs); } catch (Exception e) { throw new RuntimeException(e); }});
////        } catch (Exception e) {
...
////
////    /**
////     * ★ MAIN activity query — flow + date range.
////     * Uses index: idx_msg_phone_sent(phone, sent_at).
////     * Key fix: >= and < instead of DATE() BETWEEN.
////     */
////    public List<DashboardResponse.ActivityEventDto> findRecentActivityByFlowAndDate(
////            String flow, LocalDateTime from, LocalDateTime to) {
////        String sql =
////                "SELECT m.phone, m.direction, m.message_content, m.button_payload, m.sent_at, " +
////                        "       c.name AS customer_name " +
...
////    public List<DashboardResponse.ActivityEventDto> findRecentActivityByFlowAndDate(
////            String flow, LocalDate from, LocalDate to) {
////        return findRecentActivityByFlowAndDate(flow,
////                from.atStartOfDay(),
////                to.plusDays(1).atStartOfDay());
////    }
////
...
////    // ════════════════════════════════════════════════════════════
////
////    /** Frontend flow key → DB current_step LIKE pattern. */
////    public String flowToPattern(String flow) {
...
////        if (collection == null || collection.isBlank()) collection = extractCollection(rawStep);
...
////                .selectedBrand(defaultIfBlank(safe(rs, "selected_brand"), null))
...
////                .flow(flow.isBlank() ? detectFlow(stepName) : flow)
////                .stepName(stepName)
...
////    }
////
////    private String activityIcon(String p, String d) {
////        p = p == null ? "" : p.toUpperCase();
...
////        try { String v = rs.getString(column); return v == null ? "" : v; }
...
//
//
//    // NAYA — DashboardDataRepository.java mein add karna hai
//
//    /** "Opens" = jin messages ka status READ hua (Karix se status=read, code=102). */
...
//    }
//
//    /** "Clicks" = jitne distinct users ne kisi bhi button (INBOUND, button_payload not null) ko dabaya. */
